# Quantify the contribution of each assembly processes for the RNA/DNA dataset
# Date: 23-11-2019
# Author: Jia Xiu
...
for (data.set.name in data.set.names) {
  ...
  cat('dispersal limitation:', table(Dispersal.limilation)['TRUE'], 'within', nrow(nti.rc), 'pairwise samples\n')
  df[i, 3] <- length(Dispersal.limilation[Dispersal.limilation == TRUE])
...
df[df == 0] <- NA
df <- na.omit(df)
df$Processes <- factor(df$Processes, levels = c('Variable.selection', 'Homogeneous.selection', 'Dispersal.limitation', 
...
ggsave("Assembly_processes_Year.png", width = 12, height = 6, units = "cm", f1, scale = 1.5, 
       dpi = 300)
...
# combine plots--------------------------------------------------------------------------------------
fp <- ggarrange(f1, f2, labels = c("A", "B"), 
                common.legend = TRUE, legend = "right", ncol = 1, nrow = 2)
fp
